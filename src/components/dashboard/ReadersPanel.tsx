...
export function ReadersPanel({
	...
	const grouped = readers.reduce<Record<string, ReaderSnapshot[]>>(
		(acc, reader) => {
	...
	const handleDelete = async (viewerId: string) => {
		...
		try {
			setErasing(true);
			const response = await fetch(`/api/readers/${viewerId}`, {
				method: "DELETE",
			});
			if (!response.ok) {
			...
			const message =
				error instanceof Error ? error.message : "Unable to delete reader data";
			toast.error(message);
		} finally {
			setErasing(false);
		}
