...
import {
	DocumentBuilder,
	DashboardDocument,
} from "@/components/dashboard/DocumentBuilder";
...
export function OwnerDashboard() {
	...
	const refreshReaders = useCallback(async () => {
		...
		try {
			...
			const message =
				error instanceof Error ? error.message : "Unable to load readers";
			toast.error(message);
		} finally {
      ...
      <header className="flex flex-col gap-4 text-white md:flex-row md:items-center md:justify-between">
        ...
        >
          Sign out
			...
			{activeTab === "Documents" && (
				<DocumentsList documents={documents} onRefresh={refreshDocuments} />
			)}
			...
			{activeTab === "Readers" && (
				<ReadersPanel
					...
					isRefreshing={isRefreshingReaders}
				/>
			)}
		</div>
	);
