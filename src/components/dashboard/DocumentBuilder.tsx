...
export function DocumentBuilder({ onCreated }: Props) {
	...
	const [policies, setPolicies] = useState<DocumentSecurityPolicy>({
		...defaultPolicies,
	});
	const [identityRequirement, setIdentityRequirement] =
	...
	const handleIdentityField =
		(field: "expectedName" | "expectedPhone") =>
	...
	const handleEnforceMatch = (event: ChangeEvent<HTMLInputElement>) => {
		setIdentityRequirement((prev) => ({
			...prev,
			enforceMatch: event.target.checked,
		}));
	};
				...
				<div className="flex items-center justify-between">
					<div>
						...
						<p className="text-sm text-slate-300">
							Capture reader name/phone before the document loads. Enforce
							matching if you expect someone specific.
						</p>
					</div>
			...
			<div className="grid gap-4 lg:grid-cols-3">
				<div className="space-y-2">
					...
					<input
						...
						onChange={(event) =>
							setPermissions((prev) => ({
								...prev,
								maxViews: Number(event.target.value),
							}))
						}
						className="w-full rounded-2xl border border-white/10 bg-white/5 px-4 py-3 text-sm text-white"
					/>
				...
				<div className="space-y-2">
					...
					<input
						type="datetime-local"
						value={
							permissions.expiryDate ? permissions.expiryDate.slice(0, 16) : ""
						}
						onChange={(event) =>
							setPermissions((prev) => ({
								...prev,
								expiryDate: new Date(event.target.value).toISOString(),
							}))
						}
						className="w-full rounded-2xl border border-white/10 bg-white/5 px-4 py-3 text-sm text-white"
					/>
				...
				<div className="space-y-2">
					...
					<select
						...
						onChange={(event) =>
							setPermissions((prev) => ({
								...prev,
								securityLevel: event.target
									.value as DocumentPermissions["securityLevel"],
							}))
						}
						className="w-full rounded-2xl border border-white/10 bg-white/5 px-4 py-3 text-sm text-white"
					>
			...
			<div className="space-y-2">
				...
				<input
					...
					onChange={(event) =>
						setPolicies((prev) => ({
							...prev,
							ipRestriction: event.target.value,
						}))
					}
