...
import { SecureDocument, SessionStatus, ViolationEvent } from "@/lib/types/security";
...
export function SessionHud({ session, document, violations, revokedReason, onKill }: Props) {
    ...
    <aside className="glass-panel flex flex-col gap-4 px-5 py-4 text-sm text-slate-200">
      ...
      <div className="grid grid-cols-2 gap-3 text-xs">
        <div className="rounded-2xl border border-white/5 bg-white/5 px-3 py-3">
          <p className="text-[0.6rem] uppercase text-slate-400">Security Level</p>
          <p className="text-base font-semibold text-white">{document.permissions.securityLevel}</p>
        </div>
        <div className="rounded-2xl border border-white/5 bg-white/5 px-3 py-3">
          <p className="text-[0.6rem] uppercase text-slate-400">Max Views</p>
          <p className="text-base font-semibold text-white">{document.permissions.maxViews}</p>
        </div>
      </div>
      <div className="rounded-2xl border border-white/5 bg-slate-950/40 px-4 py-3 text-xs text-slate-300">
        {isRevoked ? (
          <span className="text-rose-200">{revokedReason ?? "Session revoked"}</span>
        ) : violations.length ? (
          <span>{violations[0].description}</span>
        ) : (
          <span>
            Viewer compliant. Camera tracking + heartbeat ({session.heartbeatMs / 1000}s) stay active.
          </span>
        )}
      </div>
    </aside>
