...
export function SecureFileViewer({ fileUrl, fileType, fileName }: Props) {
	...
	const extension = useMemo(
		() => inferExtension(fileName, fileType, fileUrl),
			...
			try {
				...
				if (extension === "doc") {
					setState({
						mode: "error",
						message:
							"Legacy .doc files cannot be previewed safely. Ask the owner to convert to .docx.",
					});
					return;
				}
				...
				setState({
					mode: "error",
					message: `Preview not available for ${
				...
				if (!cancelled) {
					setState({
						mode: "error",
						message: "Unable to render file securely.",
					});
				}
			}
		}
		...
		async function renderDocx() {
			try {
				...
				if (!cancelled) {
					setState({
						mode: "error",
						message: "Unable to render DOCX securely.",
					});
				}
			}
		}
	...
	return (
		<section className={containerClass}>
								...
								<tr key={`row-${rowIndex}`} className="border-b border-white/5">
									{row.map((cell, cellIndex) => (
										<td
											...
											className="border-r border-white/10 px-3 py-2"
										>
											{cell ?? ""}
										</td>
									))}
			...
			{state.mode === "error" && (
				<p className="text-rose-300">{state.message}</p>
			)}
		</section>
	);
}
