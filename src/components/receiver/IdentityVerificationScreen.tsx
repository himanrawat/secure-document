...
import { ViewerDeviceFingerprint, ViewerIdentityRequirement } from "@/lib/types/security";
...
export function IdentityVerificationScreen({ documentId, requirement, viewerDevice }: Props) {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const message = error instanceof Error ? error.message : "Unable to capture identity.";
      toast.error(message);
    } finally {
      setLoading(false);
    }
      ...
      >
        ...
        <div className="grid gap-4 md:grid-cols-2">
          ...
          <div className="rounded-2xl border border-white/10 bg-black/40 px-5 py-4 text-xs text-slate-300">
            ...
            <p className="mt-2">
              After submitting, the viewer switches to fullscreen, records liveness, and blocks screenshots or
              printing. Leaving fullscreen or obstructing the camera revokes access instantly.
            </p>
