	...
	return (
		<div className="relative isolate min-h-screen overflow-hidden px-6 py-12 sm:px-16">
			...
			<main className="mx-auto flex w-full max-w-6xl flex-col gap-12">
				<section className="flex gap-8 rounded-xl border border-white/5 bg-white/5 p-4 md:p-8 text-white md:flex-row flex-col">
					<div className="flex flex-col gap-4">
						<p className="text-sm uppercase tracking-[0.4em] text-slate-400">
							AegisDocs Portal
						</p>
						<h1 className="text-4xl font-semibold leading-tight sm:text-5xl">
							Share zero-leakage documents. Enforce camera guardianship,
							watermarking, and OTP gates.
						</h1>
						<p className="text-lg text-slate-300">
							Every recipient must enter the owner-generated OTP. Once unlocked,
							our viewer forces fullscreen, blocks screen recording, and streams
							camera/location telemetry back to you in real time.
						</p>
						...
						<div className="flex gap-3 flex-wrap pt-2">
							<Link
								href="/dashboard"
								className="inline-flex items-center gap-2 rounded-full bg-cyan-500 px-6 py-3 text-base font-semibold text-slate-900 transition hover:bg-cyan-400 w-full md:w-fit"
							>
								Build a secure doc
							...
							<Link
								href="#policies"
								className="inline-flex items-center rounded-full border border-white/20 px-6 py-3 text-base font-semibold text-white hover:bg-white/10 w-full md:w-fit"
							>
								Review safeguards
							</Link>
						</div>
				...
				<section id="policies" className="grid gap-6 md:grid-cols-2">
					{pillars.map((pillar) => (
						<article
							key={pillar.title}
							className="glass-panel flex h-full flex-col gap-4 px-6 py-6"
						>
					...
					<div className="flex flex-col gap-8 lg:flex-row lg:items-center lg:justify-between">
						<div>
							...
							<h3 className="mt-3 text-2xl font-semibold text-white">
								1) Owner uploads + sets OTP. 2) Receiver enters OTP. 3)
								Fullscreen secure viewer locks all exits.
							</h3>
						</div>
