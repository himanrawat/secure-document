...
export async function getDocumentById(documentId: string) {
	return readJson<StoredDocument>(
		path.join(getDocumentsDir(), `${documentId}.json`)
	);
}
...
export async function findDocumentByOtp(otp: string) {
	...
	return (
		docs.find((doc) => doc.otp.toLowerCase() === otp.toLowerCase()) ?? null
	);
}

async function deleteSessionFilesForDocument(documentId: string) {
	const sessions = await listViewerSessions();
	await Promise.all(
		sessions
			.filter((session) => session.documentId === documentId)
			.map((session) =>
				fs.rm(path.join(getSessionsDir(), `${session.token}.json`), {
					force: true,
				})
			)
	);
...
export async function createDocument(
	...
	const record: StoredDocument = {
		...
		identityRequirement:
			input.identityRequirement ?? fallbackIdentityRequirement,
		locked: false,
	};
...
export async function deleteReaderRecord(viewerId: string) {
	...
	const matches = sessions.filter(
		(session) => session.session.viewerId === viewerId
	);

	// Actually delete the session files instead of just clearing data
	await Promise.all(
		matches.map((record) => {
			const sessionPath = path.join(getSessionsDir(), `${record.token}.json`);
			return fs.rm(sessionPath, { force: true });
		})
	);
...
export async function deleteDocument(
	...
	const docPath = path.join(getDocumentsDir(), `${documentId}.json`);
	const document = await readJson<StoredDocument>(docPath);
	if (!document) {
		return false;
	}
	await fs.rm(docPath, { force: true });
	const uploadDir = path.join(getUploadsDir(), documentId);
	await fs.rm(uploadDir, { recursive: true, force: true }).catch(() => {});
	if (options?.purgeReaders) {
		await deleteSessionFilesForDocument(documentId);
	}
	emitSystemEvent({
