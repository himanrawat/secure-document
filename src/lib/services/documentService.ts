...
function mapDocument(row: DocumentRow): StoredDocument {
	...
	return {
		...
		classification:
			(row.classification as SecureDocument["classification"]) ??
			"CONFIDENTIAL",
		encryptedBlob: "",
		...
		identityRequirement:
			row.identity_requirement ?? fallbackIdentityRequirement,
		locked: row.locked,
...
export async function recordSessionViolation(
	...
	await mutateSession(token, (record) => {
		...
		// Check if violation with this ID already exists
		const existingIndex = history.violations.findIndex(
			(v) => v.id === input.violation.id
		);

		if (existingIndex >= 0) {
			// Update existing violation with new photo if provided
			if (input.photo) {
				history.violations[existingIndex] = {
					...history.violations[existingIndex],
					photo: input.photo,
				};
			}
...
export async function listReaderIdentities(
	ownerId?: string
